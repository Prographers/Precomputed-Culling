  ...
  function renderNavbar() {
    ...
    function loadNavbar() {
      ...
      $.get(navbarPath, function (data) {
        ...
        right.className = "nav level1 navbar-nav";
        right.style.float = "right";
        $("#navbar").append(right);
        const elems = $('#navbar>ul>li>a');
        for(const i in Object.keys(elems))
        {
          if (!Object.hasOwn(elems, i)) continue;
          let a = elems[i];
          if(a.title.startsWith("$"))
          {
            a.title = a.title.substr(1,a.title.length - 1);
            a.text = a.text.substr(1,a.text.length - 1);
